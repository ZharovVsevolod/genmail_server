...
class OpenSearchRetriever(OpenSearchConnection):
    ...
    def _create_filter(
        self, filter_type: FILTER_TYPES, what_to_filter: str
    ) -> list[dict]:
        match filter_type:
            case "source":
    ...
    def _format_documents(
        self, docs: list[Document], dtype: RETURN_DOCUMETNS_RETURN_TYPE
    ) -> str:
        match dtype:
            case "str":
    ...
    def similarity_search(
        ...
        found_documents = self.vectorstore.similarity_search(
            query=query, k=k, filter=actual_filter
        )

        if return_format is not None:
            found_documents = self._format_documents(
                docs=found_documents, dtype=return_format
            )
    ...
    def delete_extracted_info(self, session_id: str) -> None:
        ...
        try:
            ...
            self.client.delete(
                index=Settings.services.vectorbase.documentview_index, id=extracted_id
            )
        except:
